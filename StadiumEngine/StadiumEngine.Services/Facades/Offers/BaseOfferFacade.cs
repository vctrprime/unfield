...
internal abstract class BaseOfferFacade<T> where T : BaseOffer
{
    ...
    {
        foreach (var offersSportKind in offer.SportKinds)
        {
        ...
        {
            var path = await _imageService.Upload(image.FormFile, legalId, offer.StadiumId, $"offers/{ImageFolder}");
            offer.Images.Add(new OffersImage
            {
                Path = path,
    ...
    private async Task<List<string>> ProcessImages(List<ImageFile> passedImage, T offer, int userId)
    {
        ...
        foreach (var image in passedImage)
        {
            ...
            if (entityImage == null)
            {
                var path = await _imageService.Upload(image.FormFile, offer.Stadium.LegalId, offer.StadiumId, $"offers/{ImageFolder}");
                _imageRepository.Add(CreateImage(offer.Id, userId, path, passedImage.IndexOf(image)));
            }
            else
            {
